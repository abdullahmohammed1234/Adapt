...
def test_m9_004_policy_filters_recent_ids():
    ...
    history.from_used_ids(["CS-ALG-001", "CS-ALG-002"], lookup=lambda cid: type("M", (), {"concept_id": "c", "difficulty": 2, "challenge_type": "DIRECT", "family": cid})())
    class Item:
        def __init__(self, cid):
            self.id = cid
            self.family = cid
            self.challenge_type = "DIRECT"
    pool = [Item("CS-ALG-001"), Item("CS-ALG-009")]
    filtered = filter_repeats(pool, history, strategy=StrategyName.MAINTAIN, current_id="CS-ALG-002")
    assert all(item.id != "CS-ALG-001" for item in filtered) or len(pool) == 1
