...
def test_subjects_and_concepts_are_reachable():
    ...
    assert quantum["concept_count"] >= 12
    assert quantum["concepts"]
    assert all("status_label" in item for item in quantum["concepts"])
    assert all(item["status_label"] == "New" for item in quantum["concepts"])
    assert any(item["recommended"] for item in quantum["concepts"])
...
def test_http_navigation_endpoints():
    ...
    try:
        ...
        quantum = app.request("GET", "/api/subjects/quantum")
        assert quantum["concepts"]
        progress = app.request("GET", "/api/progress")
        assert progress["overall_available"] is False
        journey = app.request("GET", "/api/journey?subject_id=quantum")
