...
def test_subject_and_topic_selection():
    ...
    cs = service.get_subject("computer-science")
    topic_ids = {item["topic_id"] for item in cs["topics"]}
    assert "cs-algorithms" in topic_ids
...
def test_challenge_submission_and_confidence():
    service = make_service()
    view = service.create_session(topic_id="cs-algorithms", subject_id="computer-science", session_id="P7-SUB-001", max_steps=3)
    ...
    result = service.submit_response(
        view["session_id"],
        answer=view["challenge"].get("choices", ["O(log n)"])[1] if view["challenge"].get("choices") else "O(log n)",
        confidence=5,
        approach="worked",
        explanation="Binary search halves the sorted list each step, so the time is log n.",
...
def test_optional_explanation_and_feedback_render():
    service = make_service()
    view = service.create_session(topic_id="cs-algorithms", session_id="P7-SUB-002", max_steps=2)
    result = service.submit_response(
        view["session_id"],
        answer="O(log n)",
        confidence=3,
...
def test_progress_and_research_views():
    service = make_service()
    view = service.create_session(topic_id="cs-algorithms", session_id="P7-PR-001", max_steps=2)
...
def test_http_subject_session_flow():
    ...
    try:
        ...
        assert len(subjects["subjects"]) >= 6
        created = app.request(
            "POST",
            "/api/sessions",
            {"topic_id": "cs-algorithms", "subject_id": "computer-science", "max_steps": 2},
        )
        submitted = app.request(
            ...
            f"/api/sessions/{created['session_id']}/responses",
            {
                ...
                "confidence": 4,
                "approach": "worked",
                "explanation": "halve the sorted search space each step",
...
def test_legacy_topics_still_work():
    ...
    try:
        ...
        raise AssertionError("calculus topic_id must remain unsupported")
