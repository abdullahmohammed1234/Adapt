"""Deterministic multi-domain challenge catalog.

The frozen Phase 3 algebra/fractions bank is wrapped, not rewritten.
"""

from __future__ import annotations

from adapt.content.domains import calculus, chemistry, computer_science, mathematics, physics, quantum, space
...
def _wrap_phase3() -> tuple[CatalogChallenge, ...]:
    wrapped: list[CatalogChallenge] = []
    for item in PHASE3_BANK:
        if item.concept_id == "basic_algebra":
            wrapped.append(
                CatalogChallenge.from_engine(
                    item,
                    domain="mathematics",
                    topic_id="algebra",
        ...
        elif item.concept_id == "fractions":
            wrapped.append(
                CatalogChallenge.from_engine(
                    item,
                    domain="mathematics",
                    topic_id="fractions",
                    explanation="Use equivalent fractions with a common denominator.",
                )
            )
