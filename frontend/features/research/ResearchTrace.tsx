  ...
  return (
    <article className="rounded-2xl border border-white/10 bg-white/5 p-4">
      ...
      <div className="mt-3 grid gap-3 sm:grid-cols-4">
        ...
        <div>
          <p className="text-xs text-white/50">Learner state</p>
          <p className="mt-1 text-sm">
...
export function ResearchTrace({ trace }: { trace: TraceView }) {
  return (
    <section className="rounded-[var(--radius-card)] bg-research p-6 text-deep-ink">
      ...
      <div className="mt-6 grid gap-3">
        {trace.chain.length ? trace.chain.map((item) => <LinkCard key={item.step_number} item={item} />) : (
