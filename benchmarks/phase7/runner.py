...
def run_repetition_and_selection() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    state = initial_learner_state("B7", "cs_complexity")
    used = ["CS-ALG-001"]
    eligible = 0
    avoided = 0
    ok = 0
    ...
    for action, strategy in pairs * 4:
        ...
        alternatives = [
            ...
            if item.topic_id == "cs-algorithms" and item.id not in recent and item.id != challenge.challenge_id
        ]
    ...
    first = a.select(_decision(AdaptationAction.INCREASE_DIFFICULTY), state, current, ["CS-ALG-001"], StrategyName.INCREASE)
    second = b.select(_decision(AdaptationAction.INCREASE_DIFFICULTY), state, current, ["CS-ALG-001"], StrategyName.INCREASE)
    return (
